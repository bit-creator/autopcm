...
class ProgressBar:
    ...
    def message(self, string):
        def simple_msg(string):
            if len(string) > self.length:
                for line in [string[i:i+self.length] for i in range(0, len(string), self.length)]:
                    simple_msg(line)
            else:
                print(self.multiplier * ("\033[A") + "\033[A" + string.ljust(self.length) + '\n'*2)
    ...
    def draw(self):
        ...
        if len(bar) > buffer_size:
            bar = bar[:buffer_size]
        else:
            bar = bar + (buffer_size - len(bar)) * '.'

        bar2 = self.static_line2 + self.path
        if len(bar2) > buffer_size:
            bar2 = bar2[:buffer_size]
        else:
            bar2 = bar2 + (buffer_size - len(bar2)) * '.'
    ...
    async def launch(self):
        print('\033[?25l', end="")
        self.start = time.perf_counter()
        while(self.progress != self.size and self.progress2 != self.size2):
            self.draw()
